    ...
    if start_index then
      ...
      for line in output:gmatch '[^\r\n]+' do
        ...
        if task_id then
          ...
          local annotation = string.format('nvim +%s %s', line_number, file_path)
...
vim.api.nvim_set_keymap('n', '<leader>dt', "<Cmd>lua require('user_functions.tasks').mark_task_done()<CR>", { noremap = true, silent = true })
...
vim.api.nvim_set_keymap('n', '<leader>gt', "<Cmd>lua require('user_functions.tasks').go_to_task_in_taskwarrior_tui()<CR>", { noremap = true, silent = true })
