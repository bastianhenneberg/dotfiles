    ...
    for idx, line in enumerate(split_file):
        ...
        if any(map(line.__contains__, org_matches)):
            ...
            if any(map(next_line.__contains__, schedule_matches)):
                schedule_type = next(
                    substring
                    for substring in schedule_matches
                    if substring in next_line
                )
...
def get_entry_by_description(python_dict, description):
    pending_entry = next(
        (
            entry
            for entry in python_dict["pending"]
    ...
    completed_entry = next(
        (
            entry
            for entry in python_dict["completed"]
            if entry["description"] == description
        ),
        None,
    )
    ...
    for task, task_priority, state, start, delta, repeat in tasks:
        ...
        if task_id is not None:
            ...
            if start is not None and delta is not None:
                delta = start + delta
            client.task_add(
                task, priority=task_priority, due=start, until=delta, recur=repeat
            )
